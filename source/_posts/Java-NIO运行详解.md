...
[![Dml0pR.png](https://s3.ax1x.com/2020/11/18/Dml0pR.png)](https://imgchr.com/i/Dml0pR)

建立一个WebSocket连接，首先会通过TCP请求，来进行握手，然后发送HTTP请求建立连接，再使用WebSocket Protocol来进行数据的传输。通过TCP连接来进行心跳保活（大约几秒一次，会发送TCP请求）。

报文包解析：

[![DmJLSH.png](https://s3.ax1x.com/2020/11/18/DmJLSH.png)](https://imgchr.com/i/DmJLSH)

WebSocket前几位分别是：Fin、Reserved、Opcode、Mask、Payload length。之后是Masking-key、Masked payload，下面的Payload就是这次请求携带的Data。

[![DmYSTf.png](https://s3.ax1x.com/2020/11/18/DmYSTf.png)](https://imgchr.com/i/DmYSTf)
